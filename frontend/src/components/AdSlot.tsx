...
/**
 * In-feed ad component — designed to blend with listing cards.
 * Shows between listings in the browse feed.
 */
export function InFeedAd({ className = '' }: { className?: string }) {
    ...
    return (
      <div className={`rounded-2xl border border-brand-accent/20 bg-brand-accent/5 p-5 ${className}`}>
        ...
        <div className="mt-2 flex items-center gap-3">
          ...
          <div>
            <p className="text-sm font-semibold text-white">Financiering nodig?</p>
            <p className="text-xs text-surface-400">Bereken uw maandlasten — vrijblijvend</p>
          </div>
        </div>
      </div>
    );
  ...
  return <AdSlot format="in-feed" className={className} />;
}
